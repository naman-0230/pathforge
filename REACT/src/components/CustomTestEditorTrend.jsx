import { getEditorTrendData, getEditorEngagementStats } from '../utils/customTests.js';

// CustomTestEditorTrend — visualizes editor pass rate across custom test
// sessions over time. Only sessions with at least 1 editor attempt appear
// in the trend line (sessions with 0 attempts are meaningless for pass rate).

const WIDTH = 500;
const HEIGHT = 180;
const PADDING = { top: 20, right: 20, bottom: 30, left: 40 };

export default function CustomTestEditorTrend() {
  const stats = getEditorEngagementStats();
  const trendData = getEditorTrendData().filter((d) => d.passRate !== null);

  // Empty state — no editor data yet
  if (stats.attemptedInEditor === 0) {
    return (
      <div style={{
        ...
        <p>
          No editor submissions yet in your custom tests.
          <br />
    ...
    return (
      <div>
        ...
        <div style={{
          ...
          Complete more custom tests with editor submissions to see pass rate trend.
        </div>
      </div>
    );
  }
